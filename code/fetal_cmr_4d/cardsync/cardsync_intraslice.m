...
for iStk = 1:nStack
    ...
    for iLoc = 1:S(iStk).nLoc
        ...
        if any( S(iStk).H{iLoc}(:) )
            [ S(iStk).tRR{iLoc}, tRTrigger, S(iStk).hrEstRelPeakProminence(iLoc), S(iStk).hrEstPeakWidth(iLoc) ] = estimate_heartrate_xf( permute( S(iStk).Y{iLoc}, [1,2,4,3] ), S(iStk).frameDuration, 'roi', S(iStk).H{iLoc}, 'hrRange', hrRange, 'verbose', true, 'visible', 'off' );
