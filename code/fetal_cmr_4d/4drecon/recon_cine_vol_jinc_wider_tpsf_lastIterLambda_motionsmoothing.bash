...
echo "LASTITERLAMBDA=$LASTITERLAMBDA"
echo
...
CMD="mirtk reconstructCardiac $RECON $NUMSTACK $STACKS -spatialpsfjinc -thickness $THICKNESS -motion_sigma 0.222 -dofin $STACKDOFDIR/stack-transformation*.dof -slice_transformations $SLICEDOFDIR -mask $MASKCINEVOL -remote -iterations $ITER -rec_iterations $NSR -rec_iterations_last $NSRLAST -resolution $RESOLUTION -lastIter $LASTITERLAMBDA -force_exclude_stack $NUMEXCLUDESTACK $EXCLUDESTACK -force_exclude_sliceloc $NUMEXCLUDESLICE $EXCLUDESLICE -force_exclude $NUMEXCLUDEFRAME $EXCLUDEFRAME -temporalpsfgauss -numcardphase $NUMCARDPHASE -rrinterval $MEANRR -rrintervals $NUMSLICE $RRINTERVALS -cardphase $NUMFRAME $CARDPHASES -debug > log-main.txt"
echo $CMD > recon.bash
eval $CMD
