...
CMD="mirtk reconstructCardiac $RECON $NUMSTACK $STACKS -spatialpsfjinc -thickness $THICKNESS -stack_registration -target_stack $TGTSTACKNO -mask $MASKSTACKRREG -iterations 1 -rec_iterations_last 8 -resolution $RESOLUTION -lastIter $LASTITERLAMBDA -force_exclude_stack $NUMEXCLUDESTACK $EXCLUDESTACK -force_exclude_sliceloc $NUMEXCLUDESLICE $EXCLUDESLICE -force_exclude $NUMEXCLUDEFRAME $EXCLUDEFRAME -no_robust_statistics -numcardphase $NUMCARDPHASE -debug > log-main.txt"
